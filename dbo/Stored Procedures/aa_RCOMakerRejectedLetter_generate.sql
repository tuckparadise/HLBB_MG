--[aa_DisbursementCheckerRejectedLetter_generate] 'arn1'

CREATE       procedure [dbo].[aa_RCOMakerRejectedLetter_generate]
...
begin	
	...
	select @SubmissionDate = DisbursementCheckerSubmissionDate,
		@reasons = RCOMakerRejectionReason,
		@receiver = DisbursementCheckerEmailAddress
	from aaSolDisbursementSubmission_RCOMakerRejected
	where  arn = @param_arn
	...
	*/
	...
	set @lineheader = 'Disbursement rejected for ' + @param_arn 

	set @line = @line +  'Dear Disbursement Checker, <br><br>'  + CHAR(13)+CHAR(10)
	...
	set @line = @line +  'The disbursement is rejected due to the following reason below: <br><br>' + CHAR(13)+CHAR(10)
