CREATE       procedure [dbo].[aa_Report_E_RevertedByValuer]
...
@ValuerOption nvarchar(max) = null,
@ValuerOptionFlag bit = null,
...
begin
	...
	if (isnull(@ValuerOption,'') = '' and @ValuerOptionFlag = 1) set @error =  @error + 'Valuer Submission Option cannot be blank<br>'
	*/
	...
	begin							
		...
		where 				
			valuer.UpdatedDate between @DateFrom and @DateFrom
			...
			--and LOADS.ValuerCode like case when @ValCodeFlag = 1 then @ValCode else '*' end 															
			and valuer_dumaker.Status <> 'Completed'
			and Valuer_lowerOMV.arn is null 
		)
		select * 
		from cte 
		--where cte.VRStatus like case when @VRStatusFlag = 1 then @VRStatus else '*' end 
--			and 
		where cte.[VRStatus] in ('Submitted','UpdateInfo','Resubmitted','LowerOMV','Cancelled')
			and cte.[VRStatus] like case when @ValuerOptionFlag = 1 then @ValuerOption else '*' end 
