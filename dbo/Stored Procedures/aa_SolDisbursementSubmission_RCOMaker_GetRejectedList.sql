CREATE             procedure [dbo].[aa_SolDisbursementSubmission_RCOMaker_GetRejectedList]
as
begin
...
--		case when loads.ValuationRequired = 1 and vrapproved.arn is null then 'No' when loads.ValuationRequired = 1 and vrapproved.arn is not null then 'Yes' else 'N/A' end as 'VR Received',
		...
		case  when solsub.RCOMakerEmailAddress is null then rejected.RCOMakerEmailAddress else solsub.RCOMakerEmailAddress end as LastModifiedBy,
	...
	from aaSolDisbursementSubmission_DisMakerSubmission solsub		
		inner join aaSolDisbursementSubmission_RCOMakerRejected rejected on rejected.arn = solsub.arn 
