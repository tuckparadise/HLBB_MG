...
CREATE     procedure [dbo].[aa_IntDisbursementSubmission_DisChecker_GetPublicList]
as
begin
	/*
	cte1 (arn, firstRunningIDForCompletedSubmission) as
	(
	...
	*/
	...
	select 
		...
		--case when rejected.arn is not null then 'Rejected by RCO Maker' else 'New' end as 'Status',		
	...
	where 		
		...
		and isnull(IntSub.DisbursementCheckerSubmissionCompleted,0) <> 1
	...
	select 
		...
		case when loads.ValuationRequired = 1 then 'Yes' else 'no' end as 'VR Required', 
		case when loads.ValuationRequired = 1 and vrapproved.arn is null then 'No' when loads.ValuationRequired = 1 and vrapproved.arn is not null then 'Yes' else 'N/A' end as 'VR Received',
		DevSub.SolicitorSubmissionDate as SubmissionDate ,
	...
	from ddProjectDisbursementSubmission_DisMaker DevSub 
		...
		--left join aaIntDisbursementSubmission_RCOMakerRejected rejected on rejected.arn = DevSub.arn and rejected.BillingRunningID = DevSub.BillingRunningID
	...
	where 		
		DevSub.DisbursementMakerSubmissionCompleted = 1
		and isnull(DevSub.DisbursementCheckerSubmissionCompleted,0) <> 1
		and instance.arn is null 
end