CREATE procedure [dbo].[usp_DUMaker_Letter_A3_generate_list]
as
begin
	...
	AS  
	(
		select sol.arn,  loads.DateOfLOAcceptance, loads.PFSalesEmailAddress as receiveremail, 
			dbo.[ufn_DUMaker_Letter_A3_header_generate](loads.arn) as email_header,
			dbo.[ufn_DUMaker_Letter_A3_body_generate](loads.arn) as email_body
		...
		union 
			select loads.arn,  loads.DateOfLOAcceptance, loads.PFSalesEmailAddress as receiveremail, 
			dbo.[ufn_DUMaker_Letter_A3_header_generate](loads.arn) as email_header,
			dbo.[ufn_DUMaker_Letter_A3_body_generate](loads.arn) as email_body
		from sqlloads loads 
		where 
	...
	insert into SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging
	select arn, DateOfLOAcceptance, receiveremail, cte0.senderemail, email_header, email_body			
	from cte 
	CROSS JOIN cte0	

	MERGE INTO SQLJOB_DUMaker_Letter_A3_generate_list_aging AS Target  
		USING (select * from SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging)  
		...
		truncate table SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging
		truncate table SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging2		

		insert into SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging2
		...
		--into SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging2
		from SQLJOB_DUMaker_Letter_A3_generate_list_aging
		where 
			aging = 30
			and 
		...
		update SQLJOB_DUMaker_Letter_A3_generate_list_aging
		set aging = 1 
		where aging = 30

		select RowNumber, arn, DateOfLOAcceptance, receiveremail, senderemail, email_header, email_body
		from SQLJOB_DUMaker_Letter_A3_generate_list_aging_staging2
