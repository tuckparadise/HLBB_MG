CREATE       procedure [dbo].[aa_SolDisbursementSubmission_RCOChecker_GetPublicList]
as
begin
	...
	select 
		...
		case  when solsub.RCOCheckerEmailAddress is null then c_rejected.RCOCheckerEmailAddress else solsub.RCOCheckerEmailAddress end as LastModifiedBy,
		case  when solsub.UpdatedDate is null then c_rejected.UpdatedDate else solsub.UpdatedDate end as LastModifiedDate
	...
	where 
		...
		----and solsub.arn not in (select arn from K2Instance where workflow = 'RCOCheckerDisbursementApproval')
		and solsub.arn not in (select arn from MortgageAppTaskInstance where workflow = 'DisbRCOCheckerSolicitorApproval')
