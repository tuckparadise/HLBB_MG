CREATE   procedure [dbo].[ddProjectDisbursementSubmission_draft_Master_Select]
...
SELECT [PR_Stage2AFlag]
  ...
  inner join ddProjectDeveloperProjectMapping p on p.DeveloperCode = m.DeveloperCode 
	and p.ProjectCode = m.ProjectCode
  inner join ddProjectDeveloper d on d.ID = m.DeveloperCode 
  inner join ddProjectPhase ph on ph.DeveloperCode = m.DeveloperCode 
	and ph.ProjectCode = m.ProjectCode 
	and ph.PhaseCode = m.PhaseCode  
	where [DeveloperLoginID] = @ProjectCode
