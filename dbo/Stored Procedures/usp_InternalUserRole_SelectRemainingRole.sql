--[usp_InternalUserRole_SelectRemainingRole] 'jasonangot'

CREATE       procedure [dbo].[usp_InternalUserRole_SelectRemainingRole]
...
begin
	...
	SELECT al.[UserRoleID] as [UserRoleID]	
		  ,al.[UserRoleName] as [UserRoleName]
		  ,al.[UserRoleDescription] as [UserRoleDescription]
		  ,al.[AccessList] as [AccessList]
		  ,al.[CreatedDate] as [CreatedDate]
		  ,al.[UpdatedDate] as [UpdatedDate]
		  ,al.[CreatedUser] as [CreatedUser]
		  ,al.[UpdatedUser] as [UpdatedUser]		  
	FROM [dbo].[InternalUserRole_AccessList] al 
	inner join InternalUserRole r on al.[UserRoleID] = r.[UserRoleID]
	where al.UserRoleName not in (
			SELECT UserRoleName
			FROM [dbo].UserRoleMetric m
			INNER JOIN InternalUserRole r on r.UserRoleID = m.UserRoleID
			where m.UserName = @username
			--where m.UserName = 'jasonangot'
			)
