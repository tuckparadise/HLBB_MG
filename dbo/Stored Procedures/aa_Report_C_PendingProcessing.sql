--[aa_Report_C_PendingProcessing] 'Solicitor Submission', '2020-01-02', '2021-01-01', null, null, null, null, null, null, null


CREATE         procedure [dbo].[aa_Report_C_PendingProcessing]
...
begin
	...
	*/
	...
	truncate table aaMortgageReport_DocumentationPendingProcessing;
	...
	begin		
		with cte (MortgateCenterCode, CreditApplicationNumber, CollateralPurpose, DisbursementManner, ApplicantName, SolicitorCode, ValuerCode, TotalFinancingAmount,
			...
			where 
				...
				and isnull(sol_dumaker.Status,'') <> 'Completed'	
			)
			insert into aaMortgageReport_DocumentationPendingProcessing
			(
				MortgateCenterCode, 
		...
		select 
			...
			[VRStatus],
			[DocumentationSubmissionDate],			
			null as [DocumentationApprovalDate],
			null as [DocumentationApprovalMaker], 			
			null as [DocumentationRejectionDate], 
			null as [DocumentationRejectionMaker], 
			null as [DocumentationCancellationSubmissionDate], 
			null as [DocumentationCancellationApprovalDate], 
			null as [DocumentationCancellationApprovalMaker], 			
			[VRSubmissionDate],
			[VRApprovalDate],
			[VRApprovalMaker]		,
			[VRCancellationSubmissionDate],[VRCancellationApprovalDate],[VRCancellationApprovalMaker]
			, [VRLowerOMVSubmissionDate], [VRLowerOMVApprovalDate], [VRLowerOMVApprovalMaker]
		from aaMortgageReport_DocumentationPendingProcessing
		order by cast([DocumentationSubmissionDate] as datetime ) asc 
		*/
	end 	
	...
	begin		
		with cte (MortgateCenterCode, CreditApplicationNumber, CollateralPurpose, DisbursementManner, ApplicantName, SolicitorCode, ValuerCode, TotalFinancingAmount,
			...
			where 
				...
				and isnull(valuer_dumaker.Status,'') <> 'Completed'	
			)
			insert into aaMortgageReport_DocumentationPendingProcessing
			(
				MortgateCenterCode, 
