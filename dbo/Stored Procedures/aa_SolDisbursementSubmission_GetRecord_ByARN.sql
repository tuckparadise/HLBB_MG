--[aa_SolDisbursementSubmission_GetRecord_ByARN] 'TAI/RB/2019/Z0002018'
CREATE       procedure [dbo].[aa_SolDisbursementSubmission_GetRecord_ByARN]
...
begin
	...
	SELECT 
		...
		--20210321 only allow one time balance advice submission regardless of LS/PR
		case when history.arn IS NOT NULL then cast(0 as bit) else solsub.[BalanceAdviceLetterApplicableFlag] end as 'BalanceAdviceLetterApplicableFlag',      
		...
		/* 20210329 remove the data retrival from history 
		...
		*/
		...
		,solsub.PR_EarliestBillingDueDate
		--,solsub.PR_TotalBillingAmount
		,cast(solsub.PR_TotalBillingAmount as varchar) as PR_TotalBillingAmount
		,solsub.PR_AnyBridgingFinancierFlag
		,solsub.PR_BridgingFinancier
		,solsub.PR_BridgingFinancierEmailAddress

		,cast(solsub.PR_BridgingFinancierRedemptionSum as varchar) as PR_BridgingFinancierRedemptionSum
		...
		,case when solsub.LF_BilledAmount is not null then cast(solsub.LF_BilledAmount as varchar) else cast(sqlloads.LegalFeesFinancedAmount as varchar) end as 'LF_BilledAmount'						
