CREATE   procedure [dbo].[usp_ws_LOADS_STAGING_COMPLETE2]	
as
begin
		
	delete from SQLLOADS_staging2
	where arn in (select arn from sqlloads)

	if exists (
	select * FROM [dbo].SQLLOADS_staging2
	)
	begin
	...
	truncate table SQLLOADS_staging2
	/*
