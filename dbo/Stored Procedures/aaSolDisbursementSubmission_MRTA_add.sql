CREATE   procedure [dbo].[aaSolDisbursementSubmission_MRTA_add]
...
begin
	...
	*/
	...
	begin
		...
		if exists
		(
			select 1 from aaSolDisbursementSubmission_MRTA
			where arn = @arn 
		)
		begin
		...
		insert into aaSolDisbursementSubmission_MRTA
		   ...
		   values (@arn, @runningid, @FacilityAccountNumber, @PaymentMode, @PayeeID,
			...
			@PaymentReference, @PaymentDescription, getdate(),getdate())
